...
def parse_ciphipsi(infile):
    """
    Parses the given .ciphipsi file to generate residue annotation strings.

    Parameters
    ----------
    infile : str
        A .ciphipsi file generated by running the BioChemical Library (BCL).

    Returns
    -------
    Strings containing annotated residue-level attributes.
...
def main():
    ...
    # parse the given ciphipsi file
    sses, locs, oris, tops = parse_ciphipsi(infile)

    # write residue-level annotations to disk files
    ...
    with open(cmd_args.prefix + '_true_tops.fasta', 'wt') as opf:
